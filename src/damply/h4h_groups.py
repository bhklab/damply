...
def main(groups: list[str]) -> None:
	...
	all_usernames = {name for members in group_members.values() for name in members}
	...
	for username in sorted_usernames:
		...
		row = [username, userid, realname] + [
			1 if username in group_members[group] else 0 for group in groups
		]
		rows.append(row)
