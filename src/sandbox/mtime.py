...
class DirectoryModificationMetadata:
    ...
    def get_last_modification_time(self) -> int:
        ...
        print(f'Getting last modification time for {self.directory.absolute()}')
        last_mod_time = max(
    ...
    def update_last_modification_time(self, initial: bool = False) -> None:
        ...
        if not initial:
            print(f'Updated last modification time for {self.directory.absolute()}')
    ...
    def get_total_size(self) -> int:
        ...
        total_size = sum(f.stat().st_size for f in self.directory.rglob('*') if f.is_file())
        return total_size
    ...
    def display(self) -> None:
        ...
        table.add_row(
            'Last modification time', str(datetime.fromtimestamp(self.last_modification_time))
        )
        table.add_row(
            'Last modification time measured at', str(self.last_modification_time_measured_at)
        )
