...
def get_directory_size(directory: Path, show_progress: bool = True) -> ByteSize:
    if show_progress:
        with Progress(
            ...
            task = progress.add_task(f"Calculating size of {str(directory.absolute())}...", total=None)
...
def count_files(directory: Path, show_progress: bool = True) -> int:
    if show_progress:
        with Progress(
            ...
            task = progress.add_task(f"Counting files in {str(directory.absolute())}...", total=None)
