...
class DirectoryAudit:
    ...
    def _get_from_cache(self) -> bool:
        ...
        try:
            ...
            cached_last_modified = datetime.fromisoformat(cache_data.get("last_modified"))
            
            # Ensure we're comparing datetimes with the same timezone awareness
            if cached_last_modified.tzinfo is None and self.last_modified.tzinfo is not None:
                # Convert naive to aware using the same timezone as self.last_modified
                cached_last_modified = cached_last_modified.replace(tzinfo=self.last_modified.tzinfo)
            elif cached_last_modified.tzinfo is not None and self.last_modified.tzinfo is None:
                # Convert self.last_modified to aware using cached timezone
                self.last_modified = self.last_modified.replace(tzinfo=cached_last_modified.tzinfo)
