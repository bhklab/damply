...
class DirectoryAudit:
    ...
    def from_path(cls, path: Path) -> 'DirectoryAudit':
        ...
        return DirectoryAudit(
            ...
            last_accessed=datetime.fromtimestamp(
                stats.st_atime, tz=toronto_tz
            ).date(),
            last_modified=datetime.fromtimestamp(
                stats.st_mtime, tz=toronto_tz
            ).date(),
    ...
    def compute_details(self, show_progress: bool = True) -> None:
        ...
        self.last_computed = datetime.now(ZoneInfo("America/Toronto")).strftime("%Y-%m-%d %H:%M:%S")
    
    def compute_file_count(self, show_progress: bool = True) -> int:
        if self.file_count is None:
            self.file_count = count_files(directory=self.path, show_progress=show_progress)
        return self.file_count

    def compute_size(self, show_progress: bool = True) -> ByteSize:
        if self.size is None:
            self.size = get_directory_size(directory=self.path, show_progress=show_progress)
        return self.size
    ...
    def __rich_repr__(self) -> rich.repr.Result:
        ...
        yield 'file_count', self.file_count if self.file_count is not None else 'Not computed yet'
...
if __name__ == '__main__':
    ...
    audit = DirectoryAudit.from_path(Path('/cluster/projects/radiomics/Projects/IterSeg'))
    print(audit)
